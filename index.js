...
/* ─────────── راه‌اندازی ربات سرگرمی 🎪 (در همین فرایند) ─────────── */
/* ربات سرگرمی داخل همین برنامه اجرا می‌شود تا با پایان زمان اجرا،
   هر دو ربات با هم و تمیز متوقف شوند (بدون فرایند یتیم) */
try {
  require('./fun-bot');
} catch (e) {
...
/* ─────────── راه‌اندازی ربات کانال پرامپت عکس 🎨 (در همین فرایند) ─────────── */
/* هر ساعت یک پرامپت تصویرسازی هوش مصنوعی + عکس نمونه در کانال پست می‌کند.
   برای فعال‌شدن فقط متغیر محیطی PROMPT_BOT_TOKEN را ست کن. */
try {
  require('./prompt-bot');
} catch (e) {
  console.error('[prompt-bot] شروع نشد:', e && e.message);
}
